package OIDC::Lite::Demo::Server::DataHandler;
use strict;
use warnings;
use utf8;
...
sub validate_client_by_id {
    ...
    unless ( $client && $client->{client_id} eq $client_id ) {
        $client = undef;
        $client = OIDC::Lite::Demo::Server::Web::M::Client->find_by_client_id($c->db, $client_id)
            or return;
    }
...
sub create_id_token {
    my ($self) = @_;
    return unless ( $self->{request} && $c->config->{OIDC} );
    
    my $oidc_config = $c->config->{OIDC};
    my $ts = time();
    my $payload = {
...
sub create_or_update_access_token {
    ...
    return OIDC::Lite::Demo::Server::Web::M::AccessToken->create($args{auth_info});
}
...
sub get_auth_info_by_code {
    my ($self, $code) = @_;
    return OIDC::Lite::Demo::Server::Web::M::AuthInfo->find_by_code($c->db, $code);
}

sub get_auth_info_by_refresh_token {
    my ($self, $refresh_token) = @_;
    return OIDC::Lite::Demo::Server::Web::M::AuthInfo->find_by_refresh_token($c->db, $refresh_token);
}

sub get_access_token {
    my ($self, $token) = @_;
    return OIDC::Lite::Demo::Server::Web::M::AccessToken->validate($token);
}

sub get_auth_info_by_id {
    my ($self, $id) = @_;
    return OIDC::Lite::Demo::Server::Web::M::AuthInfo->find_by_id($c->db, $id);
}
